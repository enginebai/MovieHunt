...
interface MovieDao {
	...
	fun upsertMovieListResponse(
		...
		list.forEachIndexed { index, movieListResponse ->
			...
			insertMovieListId(MovieListId(category = category, movieId = movieListResponse.id,
			position = (if (currentPage <= 0) 0 else (currentPage - 1)).times(pageSize).plus(index).toLong()))
		}
	}
