...
class MovieBoundaryCallback(
    ...
    override fun onZeroItemsLoaded() {
        ...
        Timber.tag("qwer")
            .d("$category Current page ${nextPageIndex.getNextPageIndex(category.key)} onZeroItemsLoaded()")
    ...
    override fun onItemAtEndLoaded(itemAtEnd: MovieModel) {
        if (loadMoreState.value == NetworkState.LOADING || -1 == nextPageIndex.getNextPageIndex(
                category.key
            )
        ) return
        Timber.tag("qwer")
            .d("$category Next page ${nextPageIndex.getNextPageIndex(category.key)} onItemAtEndLoaded()")
        movieApi.fetchMovieList(category.key, nextPageIndex.getNextPageIndex(category.key))
            .subscribeRemoteDataSource(firstLoad = false)
    }
